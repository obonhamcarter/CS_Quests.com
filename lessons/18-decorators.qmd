...
::: {.quest-badge}
✨ QUEST 18 | Difficulty: Intermediate | Time: 7 minutes
:::
...
- 📓 **<a href="../jupyterlite/lab/index.html?path=lessons/18-decorators.ipynb" target="_blank">Open in JupyterLite</a>** - Full Jupyter environment in your browser
- ▶️ **Run code directly below** - All code cells on this page are editable and runnable
- 📥 **<a href="../files/lessons/18-decorators.ipynb" target="_blank">Download Notebook</a>** - For use in local Jupyter or Google Colab
:::

## 📖 Introduction: Enchanting Your Spells

Imagine you have a perfectly good fireball spell. It works great! But now your wizard guild wants you to **log every spell cast**, **time how long each spell takes**, and **check that the caster has enough mana** — all *without* changing any of the spells themselves.

That's exactly what **decorators** do in Python! A decorator is a special function that **wraps around another function**, adding extra behaviour before and/or after it runs — all without touching the original code.

::: {.story-box}
**⚗️ Story Time**: Think of a decorator like an enchanted ring. You put the ring on your finger (your function), and suddenly you shoot *fire* from your fingertips whenever you cast any spell. The ring added a superpower — but your original spell didn't change at all!
:::

## 💡 Explanation: Building a Decorator Step by Step
...
1. We defined `my_decorator` — it accepts a function and returns a wrapped version
...
Python has a shorthand for step 2 — the `@` symbol:

```python
@my_decorator       # ← This is exactly the same as: say_hi = my_decorator(say_hi)
def say_hi():
    print("Hi!")
```

The `@` syntax is just *syntactic sugar* — a sweeter way to write the same thing!
:::

## 🎮 Activity 1: Your First Decorator
...
def announce(func):
    ...
    def wrapper():
        ...
        func()
        print(f"✅ Spell {func.__name__} complete!\n")
    return wrapper

@announce
def fireball():
    print("   🔥 BOOM! Fireball hits the enemy!")

@announce
def ice_shield():
    print("   ❄️  Ice shield surrounds you!")

@announce
def heal():
    print("   💚 Healing light restores 30 HP!")
...
See how all three functions get the same announcement behaviour — even though you only wrote that logic *once* in `announce`?

## 🎮 Activity 2: Decorators with Arguments (Using *args and **kwargs)
...
# greet("adventurer") → "Hello, adventurer"
# With the decorator: → "HELLO, ADVENTURER!!!"
```
...
def slow_calculation(n):
    """Add up numbers from 0 to n — the slow, loopy way."""
    total = 0
    for i in range(n):
        total += i
    return total
...
def fast_calculation(n):
    """Add up numbers using the math formula — instant!"""
    return n * (n - 1) // 2
...
## 🎮 Activity 4: Stacking Decorators
...
print(msg)
# uppercase("quest complete") → "QUEST COMPLETE"
# exclaim  (...)              → "QUEST COMPLETE!!!"
# bold     (...)              → "**QUEST COMPLETE!!!**"

# Try reordering the decorators and see what changes!
```

## 🎮 Activity 5: A Practical Decorator — Mana Check

Let's build something that feels like a real game feature — a decorator that checks whether the caster has enough mana before running a spell:

```{pyodide-python}
# A mana-check decorator factory

def requires_mana(cost):
    """A decorator FACTORY — returns a decorator that checks mana."""
    def decorator(func):
        def wrapper(player, *args, **kwargs):
            if player["mana"] >= cost:
                player["mana"] -= cost
                print(f"💙 Used {cost} mana. Remaining: {player['mana']}")
                return func(player, *args, **kwargs)
            else:
                print(f"❌ Not enough mana! Need {cost}, have {player['mana']}")
        return wrapper
    return decorator

@requires_mana(cost=30)
def fireball(player, target):
    print(f"🔥 {player['name']} blasts {target} with Fireball!")

@requires_mana(cost=10)
def magic_missile(player, target):
    print(f"✨ {player['name']} fires a Magic Missile at {target}!")

@requires_mana(cost=50)
def blizzard(player, target):
    print(f"❄️  {player['name']} unleashes Blizzard on {target}!")
...
blizzard(hero, "Goblin King")      # costs 50 mana — not enough!
magic_missile(hero, "Goblin King") # costs 10 mana
print(f"\nHero mana remaining: {hero['mana']}")
```

## 🧩 Challenge: Write a Logger Decorator
...
You've just learned one of Python's most elegant features — the decorator. Now those `@` symbols you see in real code will feel like old friends! 🎉
