...
::: {.quest-badge}
🧠 QUEST 17 | Difficulty: Intermediate | Time: 7 minutes
:::
...
- 📓 **<a href="../jupyterlite/lab/index.html?path=lessons/17-higher-order-functions.ipynb" target="_blank">Open in JupyterLite</a>** - Full Jupyter environment in your browser
- ▶️ **Run code directly below** - All code cells on this page are editable and runnable
- 📥 **<a href="../files/lessons/17-higher-order-functions.ipynb" target="_blank">Download Notebook</a>** - For use in local Jupyter or Google Colab
:::

## 📖 Introduction: Functions That Use Other Functions

So far you've learned that functions are like magic spells — you write them once and cast them whenever you need them. But here's where things get *really* exciting: **in Python, functions are first-class citizens**. That means you can treat a function just like any other value — store it in a variable, put it in a list, or pass it to another function!
...
::: {.story-box}
**🧙 Story Time**: Imagine a wizard school where some spells *modify* other spells. A "Power Boost" spell doesn't do anything on its own — but when you hand it your Fireball spell, it makes Fireball twice as strong. The Power Boost spell is like a higher-order function: it takes your spell (function) and does something powerful with it!
:::

## 💡 Explanation: Functions Are Values

In Python, a function is just another kind of value. You can store it in a variable without calling it — just leave off the parentheses `()`.
...
action()                # NOW we call it — prints "Hello!"
```
...
## 🎮 Activity 1: Passing Functions as Arguments

Let's pass a function into another function!

```{pyodide-python}
# Functions as arguments — gear up!
...
Notice how `greet_player` doesn't care *which* greeting style you use — you decide at the moment you call it. That flexibility is the superpower of higher-order functions!

## 🎮 Activity 2: Writing Your Own Higher-Order Function
...
## 🎮 Activity 3: Functions Returning Functions
...
# They're independent — change one and the other stays the same
coins = 7
...
## 🎮 Activity 4: Built-in Higher-Order Functions

Python's built-in higher-order functions — `map()`, `filter()`, and `sorted()` — are powerful tools. Let's see them in action together:
...
# sorted() with a key — sort alphabetically by last letter (just for fun!)
...
## 🎮 Activity 5: A Practical Example — A Mini Pipeline
...
# Step 1: remove negatives → [5, 12, 8, 0, 7, 3]
# Step 2: square all       → [25, 144, 64, 0, 49, 9]
# Step 3: sort descending  → [144, 64, 49, 25, 9, 0]
```

## 🧩 Challenge: Build Your Own apply_twice
...
## 🎓 Summary: What Makes a Higher-Order Function?
...
- **Reusable** — one function works with many strategies
- **Readable** — the intent is clear (filter the list, transform the data)
- **Flexible** — change the behavior without rewriting the function
:::

You've unlocked one of Python's most elegant features — now onto the next adventure! 🚀
