...
::: {.quest-badge}
✨ QUEST 6 | Difficulty: Beginner | Time: 5 minutes
:::
...
- 📓 **<a href="../jupyterlite/lab/index.html?path=lessons/06-functions.ipynb" target="_blank">Open in JupyterLite</a>** - Full Jupyter environment in your browser
- ▶️ **Run code directly below** - All code cells on this page are editable and runnable
- 📥 **<a href="../files/lessons/06-functions.ipynb" target="_blank">Download Notebook</a>** - For use in local Jupyter or Google Colab
:::

## 📖 Introduction: The Spell Book
...
::: {.story-box}
**🧙 Story Time**: You're a coding wizard learning to create magical spells (functions). Each spell has a name, might need some ingredients (parameters), and produces a result. Once you create a spell, you can use it over and over with just one word!
:::
...
- **Input** (ingredients/parameters) → **Process** (your code) → **Output** (result/return value)
...
def cast_heal(health, heal_amount):
    ...
    print(f"✨ Casting Heal! +{heal_amount} HP")
    return new_health

def cast_fireball(target_name, damage):
    """Attack spell that deals damage"""
    print(f"🔥 FIREBALL at {target_name}!")
...
## 👨‍💻 Code Example: Functions Working Together
...
def attack_enemy(attacker_name, attack_power, defender_name, defense_power, critical=False):
    ...
    print(f"⚔️ {attacker_name} attacks {defender_name}!")
...
1. **Good names**: Use descriptive names that say what the function does
   - ✅ `calculate_score()`, `heal_player()`
   - ❌ `func1()`, `do_stuff()`

2. **One task**: Each function should do ONE thing well
   - ✅ Separate functions for calculating damage and applying effects
...
**The flow**: 10 → multiply by 2 → 20 → add 5 → 25
...
::: {.tip-box}
**📱 You're on Fire!** Functions are the building blocks of organized code. Master them! 🔥
:::
